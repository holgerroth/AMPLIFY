...
def parse_args():
    ...
    parser.add_argument('--output_dir', type=str,
                      default="/tmp/nvflare/amplify_finetune_seqclassification_netsurfp",
...
def main():
    ...
    # Load the biomap-research dataset
    dataset = load_dataset("biomap-research/fitness_prediction")
    dataset = dataset.cast(Features({"label": Value("float32"), "seq": Value("string")}))

    # Set tokenizer
    dataset.set_transform(lambda x: {"labels": x["label"]} | tokenizer(x["seq"], padding=True, pad_to_multiple_of=8, return_tensors='pt'))
